...
namespace SistemaVendas.Negocio
{
    public class RelatorioVendasRepository
    {
        ...
        {
            var query = @"select sum(vv.valorParticipacao)  from venda v inner join VendaVendedor vv on vv.IdVenda = v.IdVenda where vv.IdVendedor = :idVendedor and year(v.dataVenda)= :ano";
